...
const directory = mkdtempSync(join(tmpdir(), 'tratio-snapshots-'));
try {
  cpSync(join(root, 'test/cases'), directory, { recursive: true });
  const file = join(directory, 'declarations.json');
  const snapshot = JSON.parse(readFileSync(file, 'utf8'));
  snapshot[0].tokens[0][2] += ' invalid.illegal.tratio';
  writeFileSync(file, JSON.stringify(snapshot));
  const result = spawnSync(process.execPath, [join(root, 'tests/snapshots.mjs'), `--cases=${directory}`], { encoding: 'utf8', timeout: 30000 });
