...
let conciseDiagnostics = newEngine()
conciseDiagnostics.setSource(concise, "concise.iv")
let conciseProgram = newParser(newLexer(concise, conciseDiagnostics).lex(), conciseDiagnostics).parse()
doAssert conciseProgram.units[0].body.stmts[0].tag == "alias"
doAssert conciseProgram.units[0].body.stmts[1].tag == "mutable"
