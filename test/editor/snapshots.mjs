...
const names = readdirSync(directory).filter(name => name.endsWith('.iv')).sort();
assert(names.includes('library.iv'), 'Missing complex library fixture');
...
for (const name of names) {
  ...
  if (name === 'library.iv') assert(text.length >= 200, 'Complex fixture must have at least 200 lines');
  ...
  lines += actual.length;
  const assertion = join(directory, name.replace(/\.iv$/, '.json'));
