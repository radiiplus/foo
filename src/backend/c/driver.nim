...
proc build*(module: Module; mode: string; outDir: string;
    options = Native(); sourceFile = "main.iv"): Result =
  try:
    validate(options)
    ...
    if options.compile:
      ...
      let version = execCmdEx(quoteShell(compiler) & " --version")
      if version.exitCode != 0:
        raise newException(OSError, version.output)
      ...
      if options.kind == "static":
        ...
        let archived = execCmdEx(command)
